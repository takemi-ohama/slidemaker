...
async def main():
    report = "# Evaluation Report Pages 3-6 V7\n\n"
    
    # We are comparing pages 3, 4, 5, 6 of the ORIGINAL PDF
    # The output generated_pages3-6_v7.pptx contains pages 1-15 because we ran it on the full PDF
    # but we only care about verifying pages 3-6 here.
    # Page 3 is index 2, Page 4 is index 3, Page 5 is index 4, Page 6 is index 5 in 0-based index.
    # Wait, the output/temp/pdf_pages/page_003.png corresponds to page 3.
    # The output/pptx_captures/slide_p3-6_v7_003.png would correspond to page 3.
    
    # Let's generate captures for the new PPTX first.
    # Assuming user will run the capture command next, let's setup the paths assuming they exist.
    # The previous run outputted 'output/generated_pages3-6_v7.pptx' but it processed the FULL PDF (15 pages).
    # So we should look for slides 3, 4, 5, 6.
    
    slides = [
        ("output/temp/pdf_pages/page_003.png", "output/pptx_captures/slide_v7_003.png", "Slide 3"),
        ("output/temp/pdf_pages/page_004.png", "output/pptx_captures/slide_v7_004.png", "Slide 4"),
        ("output/temp/pdf_pages/page_005.png", "output/pptx_captures/slide_v7_005.png", "Slide 5"),
        ("output/temp/pdf_pages/page_006.png", "output/pptx_captures/slide_v7_006.png", "Slide 6"),
    ]
    
    for orig, gen, label in slides:
        comp_path = f"output/reports/comparisons_v7/compare_{label.replace(' ', '_')}.png"
        if create_comparison(orig, gen, comp_path, label):
            evaluation = await evaluate_comparison(comp_path, label)
            report += f"## {label}\n\n{evaluation}\n\n"
            print(f"--- {label} Evaluation ---\n{evaluation}\n--------------------------")
            
    with open("output/reports/report_v7.md", "w") as f:
        f.write(report)
    print("\nReport saved to output/reports/report_v7.md")
