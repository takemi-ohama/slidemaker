...
async def main():
    report = "# Evaluation Report Pages 4-6 V1\n\n"
    
    slides = [
        ("output/temp/pdf_pages_4-6/page_001.png", "output/pptx_captures/slide_p4-6_v1_001.png", "Slide 4 (Pillar 1)"),
        ("output/temp/pdf_pages_4-6/page_002.png", "output/pptx_captures/slide_p4-6_v1_002.png", "Slide 5 (Pillar 2)"),
        ("output/temp/pdf_pages_4-6/page_003.png", "output/pptx_captures/slide_p4-6_v1_003.png", "Slide 6 (Pillar 3)"),
    ]
    
    for orig, gen, label in slides:
        comp_path = f"output/reports/comparisons_p4-6_v1/compare_{label.replace(' ', '_').replace('(', '').replace(')', '')}.png"
        if create_comparison(orig, gen, comp_path, label):
            evaluation = await evaluate_comparison(comp_path, label)
            report += f"## {label}\n\n{evaluation}\n\n"
            print(f"--- {label} Evaluation ---\n{evaluation}\n--------------------------")
            
    with open("output/reports/report_p4-6_v1.md", "w") as f:
        f.write(report)
    print("\nReport saved to output/reports/report_p4-6_v1.md")
