...
async def main():
    report = "# Evaluation Report Pages 3-6 V9\n\n"
    
    slides = [
        ("output/temp/pdf_pages/page_003.png", "output/pptx_captures/slide_v9_003.png", "Slide 3"),
        ("output/temp/pdf_pages/page_004.png", "output/pptx_captures/slide_v9_004.png", "Slide 4"),
        ("output/temp/pdf_pages/page_005.png", "output/pptx_captures/slide_v9_005.png", "Slide 5"),
        ("output/temp/pdf_pages/page_006.png", "output/pptx_captures/slide_v9_006.png", "Slide 6"),
    ]
    
    for orig, gen, label in slides:
        comp_path = f"output/reports/comparisons_v9/compare_{label.replace(' ', '_')}.png"
        if create_comparison(orig, gen, comp_path, label):
            evaluation = await evaluate_comparison(comp_path, label)
            report += f"## {label}\n\n{evaluation}\n\n"
            print(f"--- {label} Evaluation ---\n{evaluation}\n--------------------------")
            
    with open("output/reports/report_v9.md", "w") as f:
        f.write(report)
    print("\nReport saved to output/reports/report_v9.md")
