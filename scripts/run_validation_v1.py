...
async def main():
    report = "# Evaluation Report (Pages 1-3)\n\n"
    
    slides = [
        ("output/temp/pdf_pages/page_001.png", "output/pptx_captures/slide_001.png", "Slide 1"),
        ("output/temp/pdf_pages/page_002.png", "output/pptx_captures/slide_002.png", "Slide 2"),
        ("output/temp/pdf_pages/page_003.png", "output/pptx_captures/slide_003.png", "Slide 3"),
    ]
    
    for orig, gen, label in slides:
        comp_path = f"output/reports/comparisons_v1/compare_{label.replace(' ', '_')}.png"
        if create_comparison(orig, gen, comp_path, label):
            evaluation = await evaluate_comparison(comp_path, label)
            report += f"## {label}\n\n{evaluation}\n\n"
            print(f"--- {label} Evaluation ---\n{evaluation}\n--------------------------")
            
    with open("output/reports/report_v1.md", "w") as f:
        f.write(report)
    print("\nReport saved to output/reports/report_v1.md")
